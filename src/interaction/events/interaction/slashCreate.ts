...
import { testCommandPermission } from "@/lib/testCommandPermission";

export default async function slashCreate(
  gaia: Gaia,
  ...
  if (interaction.guild) {
    ...
    if (permissionTest !== true) {
      ...
      noPermissionEmbed.description = permissionTest
        .toString()
        .includes("noperm")
        ? `You do not have permission to use this command. ${
            permissionTest.toString().includes("-")
              ? ` (Missing Permission: \`${
                  permissionTest.toString().split("-")[1]
                }\`)`
              : ""
          }`
  ...
  try {
    ...
    gaia.logger.error(
      `User ${interaction.user.username} experienced an error running command /${command.data.name}`
    );
    gaia.logger.error(x);
    return gaia.error(
      "COMMAND_ERROR",
